...
def worker(csv_text: str, label: str, soma_total):
    ...
    for row in reader:
        ...
        try:
            ...
            # inciio da regiao critica
            with soma_total.get_lock():
                soma_total.value = total + soma_total.value
            #fim da regiao critica
        except IndexError:
            pass
    print(f"[{label}] linhas={linhas}  total_parte={int(soma_total.value)}")
...
def main():
    ...
    procs = []
    soma_total = Value('d', 0.0, lock=True)
    for i, chunk in enumerate(chunks, start=1):
        csv_text_chunk = header_line + "\n" + "\n".join(chunk) if chunk else header_line
        p = Process(target=worker, args=(csv_text_chunk, f"P{i:02d}", soma_total), daemon=False)
        procs.append(p)
    ...
    print(f"\n[Main] processos={n}  linhas_totais={len(data_lines)}  Soma Total {soma_total.value}")
    print(f"\nValor esperado:{len(data_lines) * 200}")
